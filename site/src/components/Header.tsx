...
interface Subcategory {
  ...
  name: string;
  nameRu: string;
  items?: { id: number; name: string; nameRu: string; link: string }[];
}
...
const Header: React.FC = () => {
  ...
  const categories: Category[] = navigation_section?.data?.categories ?? [];
  ...
  const toggleMegaMenu = (categoryId: number | null = null) => {
    ...
    if (categoryId !== null) {
      ...
      if (firstSubcategory) {
        setActiveSubcategory(firstSubcategory.id.toString());
      }
    } else {
      ...
      >
        ...
        <Offcanvas.Body style={{ padding: 0 }}>
          <MobileResponsiveMenu
            ...
            language={language}
            navigation_section={navigation_section ?? undefined}
    ...
    <>
      <header className="ecommerce-header">
        <div
          ...
          onMouseEnter={() => toggleMegaMenu(activeCategory)}
          onMouseLeave={() => toggleMegaMenu(null)}
        >
          <Navbar
            bg="white"
            expand="lg"
            className="main-navbar py-2 border-bottom"
          >
            <Container className="d-flex align-items-center justify-content-between">
                ...
                <Nav className="d-none d-lg-flex align-items-center">
                  {categories.map((category) => (
                    <Nav.Link
                      ...
                      onMouseEnter={() => toggleMegaMenu(category.id)}
                  ...
                  <Nav.Link href="who_we_are" className="py-2">
                    {headerData?.whoWeAre}
                  </Nav.Link>
                  <Dropdown align={'end'}>
                    ...
                    <Dropdown.Menu>
                      <Dropdown.Item href='work_with_us'>
                        <span>Contact Us</span>
                      </Dropdown.Item>
                      <Dropdown.Item href='business_with_us'>
              ...
              <Nav className="d-flex flex-row align-items-center">
                ...
                <Dropdown align={'end'}>
                  <Dropdown.Toggle  variant="white"  id="dropdown-language" className="shadow-none border-0 px-2">
                  ...
                  <Dropdown.Menu className='position-absolute'>
                    ...
                    <Dropdown.Item active={language === 'ru'} onClick={() => setLanguage('ru')}>
                      RU
                    </Dropdown.Item>
                  </Dropdown.Menu>
                </Dropdown>
                ...
                <Row>
                  <Col md={3} className="border-end">
                    <h5 className="mb-3">
                      {categories.find((c) => c.id === activeCategory)?.name && 
                       getDisplayName(categories.find((c) => c.id === activeCategory)!)}
                    </h5>
                      ...
                      {categories
                        ...
                        ?.subcategories?.map((sub) => (
                          <Nav.Link
                            key={sub.id}
                            href={`#${sub.id}`}
                            className={`py-2 ${activeSubcategory === sub.id.toString() ? 'active' : ''}`}
                            onMouseEnter={() => setActiveSubcategory(sub.id.toString())}
                  ...
                  <Col md={9}>
                    <Tabs
                      ...
                      onSelect={(key) => setActiveSubcategory(key || '')}
                    >
                      {categories
                        .find((c) => c.id === activeCategory)
                        ?.subcategories?.map((sub) => (
                          <Tab
                            key={sub.id}
                            eventKey={sub.id.toString()}
                            title={getDisplayName(sub)}
                          >
                            <Row className='px-3'>
                              ...
                              <Col xs={12} className='border-top pt-4'>  
                                <h6>
                                  ...
                                  {(categories
                                    ...
                                    (brand, i) => (
                                      <a
                                        key={i}
                                        href={`#${brand}`}
                                        className="badge bg-light d-flex items-center text-dark p-2"
                                      >
                                        {brand}
                                      </a>
                                    )
